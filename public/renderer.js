...
function createBrowserDb() {
    function request(query, params = [], type = 'all') {
        const xhr = new XMLHttpRequest();
        xhr.open('POST', '/api/sql', false);
        xhr.setRequestHeader('Content-Type', 'application/json;charset=UTF-8');
        xhr.send(JSON.stringify({ query, params, type }));
        ...
        throw new Error(xhr.statusText || 'Erro na requisição SQL');
    }

    return {
        prepare(query) {
            return {
                all(...params) { return request(query, params, 'all'); },
                get(...params) { return request(query, params, 'get'); },
                run(...params) { return request(query, params, 'run'); },
            };
        },
        transaction(fn) {
            return (...args) => {
...
function mudarAba(abaSelecionada) {
    ...
    const mobileBtn = document.getElementById(`mobile-aba-${abaSelecionada}`);
    if (mobileBtn) {
        mobileBtn.classList.add('active', 'bg-emerald-50', 'text-emerald-700');
...
function abrirModalLancarFrequencia() {
    const select = document.getElementById('freq_evento_select');
    if (!select) return;

    const eventos = db.prepare(`
        SELECT e.id, e.data, e.horario, e.igreja, e.titulo
        FROM eventos e
        WHERE NOT EXISTS (
            SELECT 1 FROM escalas esc WHERE esc.id_evento = e.id AND esc.presente IS NOT NULL
        )
          AND strftime('%m', e.data) NOT IN ('05', '06')
        ORDER BY e.data ASC, e.horario ASC
    `).all();

    if (!eventos || eventos.length === 0) {
        select.innerHTML = `<option value="">Nenhuma missa pendente de frequência</option>`;
        document.getElementById('freq_tbody').innerHTML = `<tr><td colspan="4" class="py-6 text-center text-sm text-gray-500">Não há missas sem frequência a partir de julho.</td></tr>`;
    } else {
        select.innerHTML = `<option value="">Selecione uma missa</option>` + eventos.map(ev => {
            const descricao = `${ev.data.split('-').reverse().join('/')} ${ev.horario} · ${ev.titulo || ev.igreja}`;
            return `<option value="${ev.id}">${descricao}</option>`;
        }).join('');
        document.getElementById('freq_tbody').innerHTML = `<tr><td colspan="4" class="py-6 text-center text-sm text-gray-500">Escolha a missa para carregar a escala.</td></tr>`;
    }

    document.getElementById('modalLancarFrequencia').classList.remove('hidden');
}

function renderizarTabelaLancarFrequencia() {
    const select = document.getElementById('freq_evento_select');
    const tbody = document.getElementById('freq_tbody');
    if (!select || !tbody) return;

    const eventoId = select.value;
    if (!eventoId) {
        tbody.innerHTML = `<tr><td colspan="4" class="py-6 text-center text-sm text-gray-500">Selecione uma missa para carregar a lista.</td></tr>`;
        return;
    }

    const escala = db.prepare(`
        SELECT esc.id, esc.funcao_temp, esc.presente, c.nome as coroinha
        FROM escalas esc
        JOIN coroinhas c ON esc.id_coroinha = c.id
        WHERE esc.id_evento = ?
    ...
    if (!escala || escala.length === 0) {
        tbody.innerHTML = `<tr><td colspan="4" class="py-6 text-center text-sm text-gray-500">Nenhuma escala encontrada para esta missa.</td></tr>`;
        return;
    }

    statusFrequencia = {};
    tbody.innerHTML = escala.map(item => {
        const presente = item.presente === 1;
        const ausente = item.presente === 0;
        if (item.presente === 1 || item.presente === 0) {
            statusFrequencia[item.id] = item.presente;
        }
        return `
            <tr data-id="${item.id}" class="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                <td class="py-3 px-3 text-sm font-black text-gray-800">${item.coroinha}</td>
                <td class="py-3 px-3 text-xs text-gray-500 uppercase tracking-wide">${item.funcao_temp || 'Apoio'}</td>
                <td class="py-3 px-3 text-xs text-gray-600">${presente ? 'Presente' : ausente ? 'Ausente' : 'Pendente'}</td>
                <td class="py-3 px-3 text-center">
                    <div class="inline-flex rounded-full border border-gray-200 overflow-hidden">
                        <button type="button" onclick="setPresencaFrequencia(${item.id}, 1)" data-escala-id="${item.id}" data-presenca="1" class="px-3 py-2 text-xs font-black ${presente ? 'text-white bg-emerald-600' : 'text-gray-600 bg-white'} border-r border-gray-200">P</button>
                        <button type="button" onclick="setPresencaFrequencia(${item.id}, 0)" data-escala-id="${item.id}" data-presenca="0" class="px-3 py-2 text-xs font-black ${ausente ? 'text-white bg-red-600' : 'text-gray-600 bg-white'}">F</button>
                    </div>
                </td>
            </tr>`;
    }).join('');
...
function setPresencaFrequencia(id, valor) {
    statusFrequencia[id] = valor;
    const row = document.querySelector(`#freq_tbody tr[data-id='${id}']`);
    if (!row) return;

    const btnP = row.querySelector(`button[data-escala-id="${id}"][data-presenca="1"]`);
    const btnF = row.querySelector(`button[data-escala-id="${id}"][data-presenca="0"]`);
    if (btnP) btnP.className = `px-3 py-2 text-xs font-black ${valor === 1 ? 'text-white bg-emerald-600' : 'text-gray-600 bg-white'} border-r border-gray-200`;
    if (btnF) btnF.className = `px-3 py-2 text-xs font-black ${valor === 0 ? 'text-white bg-red-600' : 'text-gray-600 bg-white'}`;

    const statusCell = row.querySelector('td:nth-child(3)');
    if (statusCell) statusCell.innerText = valor === 1 ? 'Presente' : 'Ausente';
}

function salvarFrequenciaSistema() {
    const select = document.getElementById('freq_evento_select');
    if (!select || !select.value) return alert('Selecione uma missa para salvar a frequência.');

    const rows = document.querySelectorAll('#freq_tbody tr[data-id]');
    if (!rows.length) return alert('Nenhum membro carregado para essa missa.');

    let atualizados = 0;
    rows.forEach(row => {
        const id = row.dataset.id;
        if (!(id in statusFrequencia)) return;
        const presente = statusFrequencia[id] === 1 ? 1 : 0;
        db.prepare('UPDATE escalas SET presente = ? WHERE id = ?').run(presente, id);
        atualizados++;
    });

    if (atualizados === 0) return alert('Marque ao menos um membro antes de salvar.');

    statusFrequencia = {};
    carregarDadosDashboard();
    fecharModal('modalLancarFrequencia');
    alert(`✅ Frequência salva para ${atualizados} membro(s).`);
}
...
function confirmarExportacaoFrequencia() {
    try {
        ...
        alert(`✅ Planilha salva! Verifique sua pasta de downloads.`);
...
function importarMembrosExcel(event) {
    ...
    reader.onload = (e) => {
        try {
            ...
            listarCoroinhas();
        } catch (err) { alert("Erro na planilha: " + err.message); }
...
function copiarImagemEscalaZap() {
    ...
    html2canvas(elemento, {
        ...
        const nomeArquivo = `Escala_${igreja}_${dataSlug}.png`.replace(/[^a-zA-Z0-9_\-\.]/g, '_');
        baixarArquivo(dataURL, nomeArquivo);
        fecharModal('modalCompartilhar');
        alert('✅ Imagem gerada! Verifique sua pasta de downloads.');
...
const trilhaCurriculo = [
    ...
    { 
        ...
        texto: `<p>Parabéns! Você chegou ao final da sua preparação.</p>
                <p>A partir de agora, você entende o peso e a alegria de vestir a túnica. Repasse a liturgia, tire suas últimas dúvidas com seu Cerimoniário, e prepare-se para a próxima etapa da formação.</p>`,
        materiais: []
    }
];
...
function listarAspirantes() {
    ...
    if (lista.length === 0) {
        tbody.innerHTML = `<tr><td colspan="4" class="py-8 text-center text-gray-400 font-bold text-xs"><i class="fas fa-inbox text-2xl mb-2 block"></i>Nenhum candidato em formação no momento.</td></tr>`;
        return;
    }
...
function construirArvoreBinaria(lista) {
    if (!lista || lista.length === 0) return null;

    function criarNo(inicio, fim) {
        if (inicio > fim) return null;
        const meio = Math.floor((inicio + fim) / 2);
        return {
            value: lista[meio],
            index: meio,
            left: criarNo(inicio, meio - 1),
            right: criarNo(meio + 1, fim)
        };
    }

    return criarNo(0, lista.length - 1);
}

function obterNiveisDaArvore(root) {
    if (!root) return [];
    const niveis = [];
    let fila = [root];
    ...
    while (fila.length > 0) {
        ...
        fila.forEach(no => {
            if (no) {
                nivel.push(no);
                proximaFila.push(no.left, no.right);
                if (no.left || no.right) temNo = true;
            } else {
                nivel.push(null);
                proximaFila.push(null, null);
            }
        });
...
function renderizarMapaInterativo() {
    ...
    const arvore = construirArvoreBinaria(trilhaCurriculo);
    const niveis = obterNiveisDaArvore(arvore);

    const nivelHtml = niveis.map(nivel => {
        return `<div class="tree-level">${nivel.map(no => {
            if (!no) return '<div class="tree-node-wrapper"></div>';

            const modulo = no.value;
            const index = no.index;
            ...
            const cursor = isAtivo ? 'cursor-pointer hover:scale-105' : (isConcluido ? 'cursor-pointer hover:scale-105' : 'cursor-not-allowed opacity-50');
            const temFilhos = !!(no.left || no.right);

            return `
                <div class="tree-node-wrapper">
                    <div class="tree-node ${temFilhos ? 'has-children' : ''}">
                        <div ${acaoClick} title="${isAtivo ? 'Clique para aprender!' : (isConcluido ? 'Clique para revisar!' : '')}" class="tree-box ${isAtivo ? 'active' : ''} ${isBloqueado ? 'locked' : ''} ${cursor}">
                            <div class="w-16 h-16 rounded-full flex items-center justify-center ${bgCircle} ${borderClass} transition-all duration-300 relative">
                            ...
                            <div class="text-center mt-3">
                                <p class="text-[10px] font-black uppercase tracking-widest ${isConcluido ? 'text-emerald-400' : (isAtivo ? 'text-amber-400' : 'text-gray-500')}">${modulo.titulo}</p>
                                <p class="text-[9px] text-gray-400 mt-1 font-bold leading-tight">${modulo.subtitulo}</p>
                            </div>
                        </div>
                    </div>
                </div>`;
    ...
    container.innerHTML = `<div class="tree-root">${nivelHtml}</div>`;
}
...
async function conectarGoogleDrive() {
    alert('Google Drive está disponível apenas no aplicativo desktop.');
}

async function desconectarGoogleDrive() {
    alert('Google Drive não está ativo no navegador.');
}
...
async function atualizarStatusDrive() {
    ...
    if (!statusEl) return;
    statusEl.innerHTML = '<span style="color:#9ca3af;font-weight:900;">● Não disponível no navegador</span> — Google Drive no app desktop.';
    if (btnConect) {
        btnConect.disabled = true;
        btnConect.innerText = 'Google Drive não disponível';
    }
    if (btnDescon) btnDescon.classList.add('hidden');
}
